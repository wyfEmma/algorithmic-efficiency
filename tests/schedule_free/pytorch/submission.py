...
#delete
def log_params(step, z, y, framework="pytorch"):
   # Calculate x = (y - (1-b1)z) / b1
   b1 = 1.0 - HPARAMS['one_minus_beta1']
   x = (y - (1 - b1) * z) / b1
  
   # We'll save the mean and std of the first parameter layer for comparison
   with open(f"/logs/comparison_{framework}.log", "a") as f:
       f.write(f"Step {step} | z_mean: {z.mean():.6f} | y_mean: {y.mean():.6f} | x_mean: {x.mean():.6f}\n")
